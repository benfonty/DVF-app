...
sudo -u postgres psql -c "DROP DATABASE IF EXISTS dvf_202104;"
sudo -u postgres psql -c "CREATE DATABASE dvf_202104;"
sudo -u postgres psql -c "ALTER DATABASE dvf_202104 SET datestyle TO ""ISO, DMY"";"
sudo -u postgres psql -d dvf_202104 -f "create_table.sql"
...
export ANNEES=2016,2017,2018,2019,2020
export COG_MILLESIME="2020"
export CADASTRE_MILLESIME="2020-12-31"
...
do
  ...
  sudo -u postgres psql -d dvf_202104 -c "COPY dvf FROM '$DATAPATH/full_$YEAR.csv' delimiter ',' csv header encoding 'UTF8';"
done

# Ajout d'une colonne et d'index - Assez long
sudo -u postgres psql -d dvf_202104 -f "alter_table.sql"
